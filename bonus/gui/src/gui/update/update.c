...
static void draw_selected_core(cg_ui_t *ui, cw_vm_t *vm)
{
    ...
    mvwprintw(ui->stats_win, 15 + vm->prog_count, 2, "Core  < %d / %d >  :", ui->config.selected_core, vm->cores->len - 1);
    mvwprintw(ui->stats_win, 16 + vm->prog_count, 6, "Program Count  | [%d]", core->regs.pc);
    mvwprintw(ui->stats_win, 17 + vm->prog_count, 6, "Current OpCode | [%d]", vm->mem[core->regs.pc % vm->config.mem_size]);
}
...
static void draw_stats(cg_ui_t *ui, cw_vm_t *vm)
{
    ...
    mvwprintw(ui->stats_win, 11, 2, "Current Cursor     : [ % -3ld ]", ui->config.cursor);
    ...
    mvwprintw(ui->stats_win, 13, 2, "   [ % -*s ] [ Last Live ]", max_prog_name_len, "Program Name");
    for (size_t i = 0; i < vm->prog_count; i++)
        mvwprintw(ui->stats_win, 14 + i, 4, "  - [ % -*s ] [ % -9ld ]",
