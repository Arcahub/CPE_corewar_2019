...
static char cw_asm_instruct_check_args_get_type(char *parameter)
{
    int tmp = 0;
    ...
    tmp = cw_asm_instruct_check_dir_ind(parameter);
    return tmp;
}
...
static int cw_asm_instruct_check_args_param(char *parameter, op_t op,
    int index)
{
    args_type_t valid_types = op.type[index];
    args_type_t type = cw_asm_instruct_check_args_get_type(parameter);
...
int cw_asm_instruct_check_args(cw_asm_instruct_t *instruct)
{
    ...
    for (int i = 0; instruct->parameters[i] && i < MAX_ARGS_NUMBER; i++) {
        if (i > op.nbr_args ||
            cw_asm_instruct_check_args_param(parameters[i], op, i))
            return 1;
    }
    return 0;
}