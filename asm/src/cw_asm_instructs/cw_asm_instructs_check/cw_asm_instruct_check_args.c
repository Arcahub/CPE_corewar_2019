    ...
    cw_asm_error_context_t err_context, char *parameter)
{
    ...
    tmp = cw_asm_instruct_check_reg(err_context, parameter);
    if (tmp) {
        if (tmp == 2)
            return (0);
        return (T_REG);
    ...
    char *parameter, op_t op, int index)
{
    ...
    cw_asm_instruct_check_args_type_err(parameter,
            err_context, valid_types, type);
    return (1);
}
    ...
    cw_asm_instruct_t *instruct)
{
    ...
    if (instruct->instruct_code == -1)
        return (0);
    if (op.mnemonique == 0 || (!instruct->parameters[0] && op.nbr_args > 0))
        return (1);
