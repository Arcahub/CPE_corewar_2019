...
cw_param_t cw__fetch_read_int(const cw_vm_t *vm, usize_t size, usize_t *addr)
{
    ...
    param.val = read_int(vm, addr, size);
    return (param);
}
    ...
    cw_param_type_t type, usize_t *addr)
{
    ...
    switch (type) {
    case CW_PARAM_REG:
        param.val = core->regs.regs[vm->mem[*addr % vm->config.mem_size]];
        addr++;
        break;
    case CW_PARAM_DIR:
        param.val = read_int(vm, addr, vm->config.dir_size);
        break;
    case CW_PARAM_IND:
        param.val = read_int(vm, addr, vm->config.ind_size);
        break;
    }
    return (param);
}
