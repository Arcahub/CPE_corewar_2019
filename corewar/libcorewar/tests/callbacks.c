...
TestSuite(callbacks, .timeout = 1, .init = cr_redirect_stdout);

Test(callbacks, add_one_callbacks)
{
...
Test(callbacks, add_some_callbacks)
{
    cw_vm_t *vm = cw_vm_new(&DEFAULT_CONFIG, NULL, 0);
    bool err = false;
...
Test(callbacks, trigger_one_callback)
{
    cw_vm_t *vm = cw_vm_new(&DEFAULT_CONFIG, NULL, 0);
    cw_instr_t instr = {0};
    bool err = false;
    ...
    err |= cw_vm__trigger_callbacks(vm, NULL, &instr);
...
Test(callbacks, trigger_filtered_callbacks)
{
    cw_vm_t *vm = cw_vm_new(&DEFAULT_CONFIG, NULL, 0);
    cw_instr_t instr = {0};
    bool err = false;
